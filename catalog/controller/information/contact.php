...
//			$this->response->redirect($this->url->link('information/contact/success'));
		...
		if (isset($this->error['modal_success'])) {
			$data['modal_success'] = $this->error['modal_success'];
		} else {
			$data['modal_success'] = false;
		}
	...
	private function validateCallme($post) {
        ...
        if( !isset($post['phone']) || strlen($post['phone']) < 6 || !preg_match('/^[0-9]+$/i', $post['phone']) ) {
            $this->error = array('phone');
        }

        if( !isset($post['name']) || strlen($post['name']) < 3 || preg_match("/[0-9()]/", $post['name']) ) {
