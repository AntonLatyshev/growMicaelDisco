...
class ControllerNewsArticle extends Controller {
	public function getPath($ncategory_id) {
		$query = $this->db->query("SELECT c.ncategory_id, c.parent_id FROM " . DB_PREFIX . "ncategory c LEFT JOIN " . DB_PREFIX . "ncategory_description cd ON (c.ncategory_id = cd.ncategory_id) WHERE c.ncategory_id = '" . (int)$ncategory_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY c.sort_order, cd.name ASC");

		if ($query->row['parent_id']) {
			return $this->getPath($query->row['parent_id'], $this->config->get('config_language_id')) . " > " . $query->row['ncategory_id'];
		} else {
			return $query->row['ncategory_id'];
		}
	}
	...
	public function getTrueUrl($product_id) {
